...
class MapStateManager(context: Context) {
    private val mapStatePrefs: SharedPreferences = context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
    ...
    fun saveMapState(mapMie: GoogleMap) {
        ...
        editor.putInt(MAP_TYPE, mapMie.mapType)
        editor.apply()
    }
    ...
    val savedCameraPosition: CameraPosition?
        get() {
            val latitude =
                mapStatePrefs.getFloat(LATITUDE, 0f).toDouble()
            if (latitude == 0.0) {
                return null
            }
            val longitude =
                mapStatePrefs.getFloat(LONGITUDE, 0f).toDouble()
            val target = LatLng(latitude, longitude)
            val zoom = mapStatePrefs.getFloat(ZOOM, 0f)
            val bearing =
                mapStatePrefs.getFloat(BEARING, 0f)
            val tilt = mapStatePrefs.getFloat(TILT, 0f)
            return CameraPosition(target, zoom, tilt, bearing)
        }
    val savedMapType: Int
        get() = mapStatePrefs.getInt(MAP_TYPE, GoogleMap.MAP_TYPE_NORMAL)
